...
window.saveTDSSummary = async function(summaryData) {
    if (!supabaseClient) initSupabase();
    // Using upsert with onConflict on S.No to allow professional editing of existing files
    const { data, error } = await supabaseClient
        .from('tds_summaries')
        .upsert(summaryData, { onConflict: 's_no' });
    if (error) throw error;
    return data;
};

window.fetchTDSSummaries = async function() {
    if (!supabaseClient) initSupabase();
    const { data, error } = await supabaseClient.from('tds_summaries').select('*').order('created_at', { ascending: false });
    if (error) throw error;
    return data;
};

window.deleteTDSSummary = async function(id) {
    if (!supabaseClient) initSupabase();
    const { error } = await supabaseClient.from('tds_summaries').delete().eq('id', id);
    if (error) throw error;
};

// --- CLIENT MANAGEMENT & GST COMPLIANCE FUNCTIONS ---
...
window.upsertClient = async function(clientData) {
    if (!supabaseClient) initSupabase();
    const { data, error } = await supabaseClient
        .from('clients')
        .upsert(clientData, { onConflict: 'serial_no' })
        .select();
    if (error) throw error;
    return data;
};
...
window.fetchClientGSTTimeline = async function(clientSerial) {
    if (!supabaseClient) initSupabase();
    const { data, error } = await supabaseClient
        .from('gst_returns')
        .select('*')
...
window.saveClientReturn = async function(returnData) {
    if (!supabaseClient) initSupabase();
    // We will update/insert based on client_serial, type, month, fy
    const query = supabaseClient.from('gst_returns');
    let res;
    if (returnData.id) {
        res = await query.upsert(returnData);
    } else {
        res = await query.insert([returnData]);
    }
    if (res.error) throw res.error;
    return res.data;
};

window.fetchClientTDSSummary = async function(clientSerial) {
    if (!supabaseClient) initSupabase();
    // Query the unified tds_summaries table using clientSerial matching s_no
    const { data, error } = await supabaseClient
        .from('tds_summaries')
        .select('*')
        .eq('s_no', clientSerial.toString())
        .maybeSingle();
    if (error) throw error;
    return data;
};
